...
<div class="content">
    ...
    <div class="content-left">
        <a href="/matbang?action=create">create</a>
        ...
        <table class="table table-striped table-bordered" id="tableCustomer" style="width: 100%">
            ...
            <c:forEach var="temp" items="${listMatBang}">
                <tr>
                    ...
                    <td>
                        <c:choose>
                            <c:when test="${temp.idTrangThai==1}">Trống</c:when>
                            <c:when test="${temp.idTrangThai==2}">Hạ tầng</c:when>
                            <c:when test="${temp.idTrangThai==3}">Đầy đủ</c:when>
                        </c:choose>
                    </td>
                    <td><c:out value="${temp.dienTich}"/></td>
                    <td><c:out value="${temp.tang}"/></td>
                    <td>
                        <c:choose>
                            <c:when test="${temp.idLoaiMatBang==1}">Văn phòng chia sẻ</c:when>
                            <c:when test="${temp.idLoaiMatBang==2}">Văn phòng trọn gói</c:when>
                        </c:choose>
                    </td>
                    <td><c:out value="${temp.gia}"/></td>
                    <td><c:out value="${temp.ngayBatDau}"/></td>
                    <td><c:out value="${temp.ngayKetThuc}"/></td>
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <div class="alert alert-warning">
                                        Do you want delete Customer?  Name:  <strong>${temp.id}</strong>
                                        and Id: <strong>${temp.id}</strong>
                                    </div>
                                    <a href="/matbang?action=delete&id=${temp.id}" class="btn btn-success">Submit</a>
