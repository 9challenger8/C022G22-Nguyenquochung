import {Injectable} from '@angular/core';
import {Observable} from 'rxjs';
import {HttpClient} from '@angular/common/http';

@Injectable({
  providedIn: 'root'
})
export class EmployeeService {

  private URL_EMPLOYEE_SPRING = 'http://localhost:8080/employee';

  constructor(private http: HttpClient) {
  }

  // getAll(): Observable<any> {
  //   return this.http.get<any>(this.URL_EMPLOYEE_SPRING + '/searchList');
  // }
  //
  // saveCustomer(customer): Observable<any> {
  //   return this.http.post<any>(this.URL_EMPLOYEE_SPRING + '/create', customer);
  // }
  //
  // findById(id: number): Observable<any> {
  //   return this.http.get<any>(this.URL_EMPLOYEE_SPRING + '/detail/' + id);
  // }
  //
  deleteEmployee(id: number): Observable<any> {
    // @ts-ignore
    return this.http.patch<any>(this.URL_EMPLOYEE_SPRING + '/delete/' + id);
  }

  //
  // updateCustomer(id: number, customer): Observable<any> {
  //   return this.http.put<any>(this.URL_EMPLOYEE_SPRING + '/update/' + id, customer);
  // }

  getListEmployeeBySearchAndPagination(name: string, idCard: string, indexPagination: number): Observable<any> {
    return this.http.get<any>(this.URL_EMPLOYEE_SPRING + '/searchList?name=' + name + '&idCard=' + idCard +
      '&page=' + indexPagination);
  }
}
